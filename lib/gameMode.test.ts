...
import { listGameModes, loadGameMode } from './gameModeServer'

describe('game mode constructor', () => {
  it('loads the classic YAML mode from disk', async () => {
    const mode = await loadGameMode('classic')

    expect(mode).toMatchObject({
      id: 'classic',
      bidding: { wordCount: 5, minBid: 6, maxBid: 25 },
      stacks: { wordLimit: 20, allCorrectBonus: 5 },
      money: { wordCount: 10, wordLimit: 25, jackpotPoints: 25 },
    })
  })

  it('loads additional YAML modes with distinct rule profiles', async () => {
    await expect(loadGameMode('quick-party')).resolves.toMatchObject({
      id: 'quick-party',
      bidding: { contests: 1, successPoints: 8 },
      money: { wordCount: 8 },
    })

    await expect(loadGameMode('chaos')).resolves.toMatchObject({
      id: 'chaos',
      challenge: { frequency: 'high', maxPerGame: 8 },
      stacks: { allCorrectBonus: 8 },
    })
  })

  it('validates every bundled YAML mode definition', async () => {
    const modeDir = path.join(process.cwd(), 'gamemodes')
    const files = (await readdir(modeDir)).filter(file => file.endsWith('.yaml'))

    expect(files).toHaveLength(10)
    for (const file of files) {
      const parsed = parse(await readFile(path.join(modeDir, file), 'utf8'))
      expect(validateGameModeDefinition(parsed), file).toEqual([])
    }
  })

  it('uses YAML filenames as route ids when listing modes', async () => {
    const filePath = path.join(process.cwd(), 'gamemodes', 'route-only-test.yaml')
    await writeFile(filePath, [
      'id: internal-test-id',
      'name: Route Only Test',
      'shortName: ROUTE',
      'description: Test mode with a mismatched YAML id.',
      'roundCount: 3',
      'timing:',
      '  turnSeconds: 30',
      '  moneySeconds: 45',
    ...
    try {
      const modes = await listGameModes()
      const listed = modes.find(mode => mode.name === 'Route Only Test')

      expect(listed?.id).toBe('route-only-test')
      expect(await loadGameMode(listed?.id)).toMatchObject({ id: 'internal-test-id' })
    } finally {
